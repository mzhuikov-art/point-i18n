...
import { IStorageService, IConfigService, AuthResponse, AuthErrorResponse, FetchLocalesResponse, CreateKeyRequest, CreateKeyResponse, CreateKeyErrorResponse, UpdateKeyResponse, SearchKeysResponse, ProjectsResponse } from '../types/index';
...
export class ApiService {
    ...
    constructor(configService: IConfigService, storageService?: IStorageService) {
    ...
    private async getUserInfoUrl(): Promise<string> {
        const baseUrl = await this.configService.getApiBaseUrl();
    ...
    private async getCreateKeyUrl(projectKey: string): Promise<string> {
        const baseUrl = await this.configService.getApiBaseUrl();
        return `${baseUrl}/api/v1/proxy/localization/api/localization/${projectKey}/new`;
    }

    private async getUpdateKeyUrl(projectKey: string): Promise<string> {
        const baseUrl = await this.configService.getApiBaseUrl();
        return `${baseUrl}/api/v1/proxy/localization/api/localization/${projectKey}`;
    }

    private async getSearchKeysUrl(projectKey: string): Promise<string> {
        const baseUrl = await this.configService.getApiBaseUrl();
        return `${baseUrl}/api/v1/proxy/localization/api/localization/${projectKey}`;
    }
    ...
    async authenticate(username: string, password: string): Promise<AuthResponse> {
        ...
        const res = await fetch(await this.getAuthUrl(), {
            headers: {
                "endpoint": "KC_AUTH",
